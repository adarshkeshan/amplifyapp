...
export default function AdlineUpdateForm(props) {
  const {
    id: idProp,
    adline: adlineModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = adlineRecord
      ? { ...initialValues, ...adlineRecord }
      : initialValues;
    setDim_ad_id(cleanValues.dim_ad_id);
    setAd_name(cleanValues.ad_name);
    setAd_cfid(cleanValues.ad_cfid);
    setAd_url(cleanValues.ad_url);
    setAd_status(cleanValues.ad_status);
    setMs_ss(cleanValues.ms_ss);
    setLine_item_type(cleanValues.line_item_type);
    setSales_vertical(cleanValues.sales_vertical);
    setAdvertiser_vertical(cleanValues.advertiser_vertical);
    setAdvertiser_cfid(cleanValues.advertiser_cfid);
    setAdvertiser_name(cleanValues.advertiser_name);
    setService_type(cleanValues.service_type);
    setAdvertiser_industry(cleanValues.advertiser_industry);
    setAdvertiser_sub_industry(cleanValues.advertiser_sub_industry);
    setCampaign_cfid(cleanValues.campaign_cfid);
    setCampaign_name(cleanValues.campaign_name);
    setCampaign_goal(cleanValues.campaign_goal);
    setCampaign_goal_category(cleanValues.campaign_goal_category);
    setIs_supply_3px(cleanValues.is_supply_3px);
    setIs_supply_ono(cleanValues.is_supply_ono);
    setIs_supply_aps(cleanValues.is_supply_aps);
    setIs_endemic(cleanValues.is_endemic);
    setIs_onsite(cleanValues.is_onsite);
    setIs_offsite(cleanValues.is_offsite);
    setBase_bid(cleanValues.base_bid);
    setDim_date(cleanValues.dim_date);
    setImpressions(cleanValues.impressions);
    setSpend(cleanValues.spend);
    setClicks(cleanValues.clicks);
    setM2_considerations_click(cleanValues.m2_considerations_click);
    setPurchases(cleanValues.purchases);
    setPurchase_amounts(cleanValues.purchase_amounts);
    setAvg_max_bid(cleanValues.avg_max_bid);
    setNumber_bid_request_eligible(cleanValues.number_bid_request_eligible);
    setNumber_bid_request_won_in_cs(cleanValues.number_bid_request_won_in_cs);
    setNumber_impressions(cleanValues.number_impressions);
    setTotal_impression_at_max_bid(cleanValues.total_impression_at_max_bid);
    setInternal_win_rate(cleanValues.internal_win_rate);
    setExternal_win_rate(cleanValues.external_win_rate);
    setAverage_bidamount_among_bids(cleanValues.average_bidamount_among_bids);
    setErrors({});
  };
  const [adlineRecord, setAdlineRecord] = React.useState(adlineModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Adline, idProp)
        : adlineModelProp;
      setAdlineRecord(record);
    };
    queryData();
  }, [idProp, adlineModelProp]);
  React.useEffect(resetStateValues, [adlineRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Adline.copyOf(adlineRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AdlineUpdateForm")}
      {...rest}
    >
      <TextField
        label="Dim ad id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || adlineModelProp)}
